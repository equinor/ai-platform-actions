...
| [`inner-loop/test_batch_lifecycle.py`](../inner-loop/test_batch_lifecycle.py) | 22 | Batch endpoint and deployment creation, tag handling, GitHub outputs, idempotent default-deployment changes, concurrent-change protection, update verification, promotion/rollback, and pinned deployment invocation. |
| [`inner-loop/test_deploy_feature_store.py`](../inner-loop/test_deploy_feature_store.py) | 14 | Managed feature store client routing, feature set and entity deployment, version assignment, YAML validation, output references, and preservation of existing `.amlignore` files. |
...
| [`inner-loop/test_share.py`](../inner-loop/test_share.py) | 2 | Registry share verification retries, including delayed ARM visibility and the diagnostic raised after the retry window expires. |
