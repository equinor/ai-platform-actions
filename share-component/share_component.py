...
def print_execution_results(shared_component, resource_id, component_ref_output, registry_name):
    ...
    print("")
    print("✅ Component shared successfully")
    print(f"Name: {shared_component.name} | Version: {shared_component.version}")
    print(f"Reference: {component_ref_output}")
    print(f"Registry: {registry_name}")
    if hasattr(shared_component, 'tags') and shared_component.tags:
        tags_list = [f"{k}={v}" for k, v in shared_component.tags.items()]
